...
public class StopOrderHandlerTest {
    ...
    public void setup(){
        ...
        noFillOrderBuy = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Buy, OrdType.Stop, 800, 0, 200);
        noFillOrderSell = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Sell, OrdType.Stop, 1000, 0, 50);
    ...
    public void test_fill_on_quote(){
        order = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Buy, OrdType.Stop, 50, 0, 98);
        handler.process(order, quote);
        verify(executor, times(1)).execute(order, quote.ask, quote.askSize);

        reset(executor);

        order = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Sell, OrdType.Stop, 100, 0, 95);
        handler.process(order, quote);
    ...
    public void test_fill_on_trade(){
        order = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Buy, OrdType.Stop, 50, 0, 99);
        ...
        order = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Sell, OrdType.Stop, 100, 0, 99);
        handler.process(order, trade);
        verify(executor, times(1)).execute(order, trade.price, order.ordQty);
        reset(executor);


        order = SampleEventFactory.createOrder(SampleEventFactory.testInstrument.instId, Side.Sell, OrdType.Stop, 100, 0, 100);
        handler.process(order, trade);
        verify(executor, times(1)).execute(order, trade.price, order.ordQty);
    }
