...
public class MultiEventHandlerGroup<T>
{
     ...
     *
     * @param groups the processors to combine.
     * @return a new MultiEventHandlerGroup combining the existing and new processors into a single dependency group.
     */
    public MultiEventHandlerGroup<T> and(final Tuple2<MultiEventProcessor, EventHandler<T>>[] groups)
    {
        return and(disruptor.handleEventsWith(groups));
    }
     ...
     */
    public MultiEventHandlerGroup<T> then(final Tuple2<MultiEventProcessor, EventHandler<T>>... groups)
    {
     ...
     * @return a {@link MultiEventHandlerGroup} that can be used to set up a event processor barrier over the created event processors.
     */
    public MultiEventHandlerGroup<T> handleEventsWith(final Tuple2<MultiEventProcessor, EventHandler<T>>... groups)
    {
